(() => {
  ...
  function drawVeinPath(g, pts, color, alpha = 1, width = 2) {
  ...
  function createVeinsLayer(banner) {
    const c = new PIXI.Container();
    c._veins = makeVeinData(banner);
    c._graphics = [];
  ...
  function redrawVeins(layer, pulse = 0, wiggle = 0) {
    ...
    for (let i = 0; i < layer._veins.length; i++) {
      ...
      drawVeinPath(gGlow, pts, vein.glow, 0.24 + pulse * 0.16, vein.thickness + 5.0);
      drawVeinPath(gMain, pts, vein.color, 0.96, vein.thickness + pulse * 0.60);

      gGlow.alpha = clamp01(vein.progress * 1.15);
      gMain.alpha = clamp01(vein.progress * 1.10);
  ...
  function createTentacleLayer(banner, { front = false } = {}) {
    const c = new PIXI.Container();
    c._front = front;
    c._tentacles = [];
  ...
  function drawTentacle(g, tentacle, time, intensity = 1) {
    g.clear();

    const segs = 12;
    const pts = [];
    const p = clamp01(tentacle.progress);
    if (p <= 0) return;
  ...
  function spawnSpore(banner, {
    ...
    const radius = preTentacle
      ? randomBetween(2.6, big ? 6.8 : 5.2)
      : big
        ? randomBetween(2.8, 6.0)
        : randomBetween(1.6, 3.6);

    const alpha = preTentacle
      ? randomBetween(0.48, 0.92)
      : big
        ? randomBetween(0.36, 0.72)
        : randomBetween(0.24, 0.58);
    ...
    banner.spawnParticle({
      ...
      life: preTentacle
        ? randomBetween(1000, 1900)
        : big
          ? randomBetween(950, 1750)
          : randomBetween(750, 1450),
      scaleFrom: preTentacle
        ? randomBetween(1.0, 1.45)
        : randomBetween(0.90, 1.25),
      scaleTo: preTentacle ? 0.20 : 0.12
    });
  }

  function spawnSporesBurst(banner, count = 14, aroundCenter = false, preTentacle = false) {
    for (let i = 0; i < count; i++) {
      spawnSpore(banner, {
        x: aroundCenter
          ? randomBetween(-70, 70)
          : randomBetween(-banner.totalWidth * 0.44, banner.totalWidth * 0.44),
        y: aroundCenter
          ? randomBetween(-30, 30)
          : randomBetween(-banner.height * 0.44, banner.height * 0.44),
  ...
  function createMistLayer(banner) {
    const c = new PIXI.Container();
    c._blobs = [];
    ...
    for (let i = 0; i < 8; i++) {
      ...
      c._blobs.push({
        ...
        rx: randomBetween(32, 70),
        ry: randomBetween(14, 30),
        phase: Math.random() * Math.PI * 2,
        drift: randomBetween(4, 10)
      });
    }
  ...
  function redrawMist(layer, elapsed, alphaMul = 1) {
  ...
  globalThis.CriticalRubanEffects.registerRubanEffect({
    ...
    setup(banner) {
      banner.addEffectLayer("abyssMist", createMistLayer(banner), {
        parent: "bodyGroup",
        alpha: 0
      });

      banner.addEffectLayer("abyssVeins", createVeinsLayer(banner), {
        parent: "bodyGroup",
        alpha: 0
      });

      banner.addEffectLayer("tentaclesBack", createTentacleLayer(banner, { front: false }), {
        parent: "bodyGroup",
        alpha: 0
      });

      banner.addEffectLayer("tentaclesFront", createTentacleLayer(banner, { front: true }), {
        parent: "fx",
        alpha: 0
      });
    },
    ...
    onHold(banner, t, dtMS) {
      ...
      const weird = jitter(banner.floatSeed, time);
      const weird2 = jitter(banner.shineSeed + 2.4, time * 1.2);
      ...
      banner.motion.tint = mixHex(0xffffff, 0x8760ae, 0.12 + pulse * 0.10);
      banner.innerGlow.alpha = 0.40 + pulse * 0.06;
      banner.shine.alpha = Math.max(0, banner.shine.alpha * 0.82);

      if (mist) {
        mist.alpha = 0.46 + pulse * 0.10;
        redrawMist(mist, banner.elapsed, 1.15);
      }

      if (veins) {
        veins.alpha = 0.24 + pulse * 0.10;
        for (const v of veins._veins) v.progress = 0.24 + pulse * 0.06;
        redrawVeins(veins, pulse, time * 0.9);
      }

      if (Math.random() < 0.34) {
        spawnSpore(banner, {
          x: randomBetween(-banner.mainWidth * 0.44, banner.mainWidth * 0.44),
          y: randomBetween(-banner.height * 0.40, banner.height * 0.36),
          dark: Math.random() < 0.35,
          preTentacle: true
        });
      }
    ...
    onPrepareExit(banner) {
      ...
      if (mist) {
        mist.alpha = 0.52;
        redrawMist(mist, banner.elapsed, 1.2);
      }

      if (veins) {
        veins.alpha = 0.42;
        for (const v of veins._veins) v.progress = 0.30;
        redrawVeins(veins, 0.26, banner.elapsed / 1000);
      }
      ...
      spawnSporesBurst(banner, 50, false, true);
    },

    onExit(banner, t, dtMS) {
      const infectT = clamp01(t / 0.28);
      const graspT = clamp01((t - 0.28) / 0.38);
      const implodeT = clamp01((t - 0.66) / 0.34);
      ...
      if (t <= 0.28) {
        const e = easeOutCubic(infectT);
        const pulse = (Math.sin(sec * 8.3) + 1) * 0.5;
        const mad = jitter(banner.floatSeed + 1.3, sec * 1.8);

        banner.root.alpha = 1;
        banner.root.position.set(
          banner.baseX + mad * 5.2,
          banner.baseY + jitter(banner.shineSeed + 0.6, sec * 1.55) * 2.9
        );
        ...
        banner.motion.tint = mixHex(0xffffff, 0x432b54, e * 0.62);
        banner.innerGlow.alpha = lerp(0.40, 0.16, e);
        banner.bodyGroup.alpha = lerp(1, 0.86, e);
        banner.shine.alpha = 0;

        if (mist) {
          mist.alpha = lerp(0.52, 0.76, e);
          redrawMist(mist, time, 1.35 + e * 0.20);
        }

        if (veins) {
          veins.alpha = lerp(0.42, 1.0, e);
          for (let i = 0; i < veins._veins.length; i++) {
            const v = veins._veins[i];
            const localDelay = i * 0.05;
            v.progress = clamp01((infectT - localDelay) / (1 - localDelay));
          }
          redrawVeins(veins, pulse * 1.15, sec * 2.2);
        }

        if (Math.random() < 0.75) {
          spawnSpore(banner, {
            x: randomBetween(-banner.mainWidth * 0.42, banner.mainWidth * 0.42),
            y: randomBetween(-banner.height * 0.36, banner.height * 0.36),
        ...
        if (infectT > 0.12 && Math.random() < 0.20) {
          spawnSporesBurst(banner, 3, false, true);
        }
      ...
      if (t <= 0.66) {
        ...
        banner.motion.scale.set(
          banner.baseScale * lerp(1, 0.955, e),
          banner.baseScale * lerp(1, 1.05, e)
        );
        banner.motion.rotation = banner.baseRotation + Math.sin(sec * 10.5) * 0.015;

        banner.motion.tint = mixHex(0x432b54, 0x140d17, e * 0.55);
        banner.innerGlow.alpha = lerp(0.16, 0.06, e);
        banner.bodyGroup.alpha = lerp(0.86, 0.72, e);

        if (mist) {
          mist.alpha = lerp(0.76, 0.90, e);
          redrawMist(mist, time, 1.45);
        }

        if (veins) {
          veins.alpha = 1.0;
          for (const v of veins._veins) v.progress = 1;
          redrawVeins(veins, pulse * 1.25, sec * 2.9);
        }

        if (back) {
          back.alpha = lerp(0, 1.0, e);
          for (let i = 0; i < back._tentacles.length; i++) {
            const tent = back._tentacles[i];
            tent.progress = clamp01((graspT - i * 0.07) / (0.82 - i * 0.07));
            drawTentacle(tent.g, tent, time, 1.12);
          }
        }

        if (front) {
          front.alpha = lerp(0, 1.0, e);
          for (let i = 0; i < front._tentacles.length; i++) {
            const tent = front._tentacles[i];
            tent.progress = clamp01((graspT - 0.10 - i * 0.07) / (0.74 - i * 0.07));
            drawTentacle(tent.g, tent, time, 1.28);
          }
        }

        if (Math.random() < 0.30) {
          spawnSpore(banner, {
            x: randomBetween(-banner.mainWidth * 0.26, banner.mainWidth * 0.26),
            y: randomBetween(-banner.height * 0.24, banner.height * 0.24),
      ...
      const sx = banner.baseScale * lerp(0.955, 0.34, e);
      const sy = banner.baseScale * lerp(1.05, 0.18, e);
      banner.motion.scale.set(sx, sy);
      banner.motion.rotation = banner.baseRotation + lerp(0.015, 0.16, e);

      banner.motion.tint = mixHex(0x140d17, 0x050306, e * 0.65);
      banner.innerGlow.alpha = lerp(0.06, 0, e);
      banner.bodyGroup.alpha = lerp(0.72, 0.06, e);

      if (mist) {
        mist.alpha = lerp(0.90, 0.08, e);
        redrawMist(mist, time, 1.5 - e * 0.7);
      }

      if (veins) {
        veins.alpha = lerp(1.0, 0.10, e);
        for (const v of veins._veins) v.progress = 1;
        redrawVeins(veins, 0.45, sec * 3.1);
      }

      if (back) {
        back.alpha = lerp(1.0, 0.08, e);
        for (const tent of back._tentacles) {
          tent.progress = lerp(1, 0.76, e);
          drawTentacle(tent.g, tent, time, 1.15 + (1 - e) * 0.2);
        }
      }

      if (front) {
        front.alpha = lerp(1.0, 0.10, e);
        for (const tent of front._tentacles) {
          tent.progress = lerp(1, 0.80, e);
          drawTentacle(tent.g, tent, time, 1.32 + (1 - e) * 0.24);
        }
      }
      ...
      if (implodeT > 0.14 && Math.random() < 0.26) {
        spawnSporesBurst(banner, 4, true, false);
      }
    },
