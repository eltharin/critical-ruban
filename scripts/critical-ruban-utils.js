...
function gCircle(g, x, y, radius, fillColor = null, fillAlpha = 1, lineWidth = 0, lineColor = 0x000000, lineAlpha = 1) {
  if (lineWidth > 0) g.lineStyle(lineWidth, lineColor, lineAlpha);
  if (fillColor !== null && fillColor !== undefined) g.beginFill(fillColor, fillAlpha);
...
function gEllipse(g, x, y, rx, ry, fillColor = null, fillAlpha = 1, lineWidth = 0, lineColor = 0x000000, lineAlpha = 1) {
  if (lineWidth > 0) g.lineStyle(lineWidth, lineColor, lineAlpha);
  if (fillColor !== null && fillColor !== undefined) g.beginFill(fillColor, fillAlpha);
...
function gPoly(g, points, fillColor = null, fillAlpha = 1, lineWidth = 0, lineColor = 0x000000, lineAlpha = 1) {
  if (lineWidth > 0) g.lineStyle(lineWidth, lineColor, lineAlpha);
  if (fillColor !== null && fillColor !== undefined) g.beginFill(fillColor, fillAlpha);
...
function gLineStyle(g, width, color, alpha = 1) {
  g.lineStyle(width, color, alpha);
  return g;
}

function gStar(g, x, y, points, outerRadius, innerRadius, fillColor = null, fillAlpha = 1, lineWidth = 0, lineColor = 0x000000, lineAlpha = 1) {
  const pts = [];
  ...
  return gPoly(g, pts, fillColor, fillAlpha, lineWidth, lineColor, lineAlpha);
}
...
function playRubanSound(type) {
  ...
  try {
    ...
    console.warn(`${MODULE_ID} | Impossible de jouer le son :`, err);
  }
}

function acquireBannerSlot() {
  const slots = globalThis.__critBannerSlots;
  const now = Date.now();
...
function releaseBannerSlot(index) {
  globalThis.__critBannerSlots.delete(index);
}
...
function cssToHex(value) {
  const normalized = normalizeHexColor(value) ?? "#8b0000";
